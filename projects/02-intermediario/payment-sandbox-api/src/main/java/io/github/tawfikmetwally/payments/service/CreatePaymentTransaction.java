...
public class CreatePaymentTransaction {
    ...
    @Transactional
    public CreatePaymentResult execute(CreatePaymentCommand command, String requestHash) {
        var existingRecord = findRecord(command);
        if (existingRecord.isPresent()) {
            return toReplayResult(existingRecord.get(), requestHash);
        }
